...
var app = {
	...
	init: function () {
		...
		app.storage.getData(function (data) {
	...
	tools: {
		...
		statusByType: function (type) {
			...
			for (var k = 0; k < l; k++) {
				if (
					app.data.array[k]["type"] == type &&
					app.data.array[k]["checked"] == true
				) {
					items.ranked.push(app.data.array[k]);
				} else if (
					app.data.array[k]["type"] == type &&
					app.data.array[k]["checked"] == false
				) {
	...
	render: {
		...
		views: {
			...
			mr: function () {
				var template = require("./../views/pages/mr.hbs");
				var html = template(app.mr.create());
				$("#mr-placeholder").html(html);
				$(".action-close-modal").on("click", function () {
					app.tools.closeModal();
				});
			},
		...
		page: {
			...
			mr: function () {
				app.render.views.mr();
				$("#mr-modal").foundation("reveal", "open", app.config.modal);
				app.tools.closeMenu();
				// Bind click on item name
				$("#mr-placeholder .action-toggle-info").on("click", function () {
					var item = $(this).data("item");
					window["app"]["item"]["toggleInfo"](item);
				});
				// Track event
				gtag("event", "Open modal: MR", {
					event_category: "Open modal",
					event_label: "Open modal: MR",
				});
			},
			news: function () {
	...
	search: {
		...
		updateIndicator: function () {
			...
			if (string.length) {
				$("#search-indicator")
					.find("#search-indicator-string")
					.html("Searching for <b>" + string + "</b>. ");
			} else {
	...
	item: {
		...
		toggleInfo: function (name) {
			...
			$("#item-info-placeholder")
				.html(html)
				.foundation("reveal", "open", app.config.modal);
			app.tools.closeMenu();
		...
		check: function (name, state) {
			...
			app.storage.saveData(function () {
				...
				app.search.updateIndicator();
				$(".action-item-check[data-item-check='" + name + "']").on(
					"click",
					function () {
						var item = $(this).data("item-check");
						var state = JSON.parse($(this).data("state"));
				...
				$(".action-toggle-info[data-item='" + name + "']").on(
					"click",
					function () {
						var item = $(this).data("item");
						window["app"]["item"]["toggleInfo"](item);
	...
	stats: {
		create: function () {
			var categories = [
				...
				{
					label: "Robotic",
					types: ["Sentinel", "Sentinel Weapon"],
				},
				{
					label: "Companions",
					types: ["Companion"],
			...
			for (var key in categories) {
				...
				for (var type in categories[key]["types"]) {
					var typeStatus = app.tools.statusByType(
						categories[key]["types"][type]
					);
				...
				var total =
					typeDataTotal + (typeDataRanked.length + typeDataUnranked.length);
				var item = {
